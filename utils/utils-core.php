...
class Utils_Core
{
  ...
  {
    $encryption_key = ZIPPY_MENU_ORDER_PREFIX;
  ...
  {
    return array(
      'id'          => ZIPPY_MENU_ORDER_PREFIX . '_divider',
      'name'       => __('', ZIPPY_MENU_ORDER_PREFIX . 'woocommerce-settings-tab'),
      'type'        => 'title',
      'desc' => '<hr>'
    );
  }
  ...
  public static function get_template($template_name, $vars = array(), $absolute_path = '', $relative_path = '')
  {
    ...
    if (file_exists(ZIPPY_MENU_ORDER_DIR_PATH . $relative_path . $template_name)) {
      $template = ZIPPY_MENU_ORDER_DIR_PATH . $relative_path . $template_name;
    }

    $template = apply_filters_deprecated(ZIPPY_MENU_ORDER_PREFIX . '\util\get_template\path_file', [$template, $vars], '1.0.0', ZIPPY_MENU_ORDER_PREFIX . '\util\get_template\template');
    $template = apply_filters(ZIPPY_MENU_ORDER_PREFIX . '\util\get_template\template', $template, $template_name, $absolute_path, $relative_path);
